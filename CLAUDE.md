...
*   **Topo (Header):** Título em uppercase (`muted`) e Badge de status com ícone.
*   **Centro (Value):** Valor principal em destaque (`Inter 300` ou `Inter 700`) com cor vinculada ao status.
*   **Base (Footer):** Subtexto contextual ou descrição da meta em fonte reduzida (`dim`).
